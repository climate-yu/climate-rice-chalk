...
def main():
    ...
    if best_weighted_bundle is not None:
        ...
        plot_true_vs_pred_extended(best_weighted_bundle["y_train"], best_weighted_bundle["m2_train_pred"], best_weighted_bundle["y_test"], best_weighted_bundle["m2_test_pred"], "Selected M3 model", EXTENDED_OUTPUT_DIR, "best_R5CW_M2")
        ...
        weight_df = plot_weight_outputs(best_weighted_record, teacher_clean, best_weighted_bundle["test_df"], EXTENDED_OUTPUT_DIR)
        plot_grid_heatmaps_and_rankings(result_df, EXTENDED_OUTPUT_DIR)

        # Additional learning-process and learning-effect figures for the final knowledge-guided model.
        # These outputs are diagnostic summaries only and do not affect model training or selection.
        screen_outputs = plot_model_screening_diagnostics(result_df, EXTENDED_OUTPUT_DIR)
        alpha_outputs = plot_alpha_search_diagnostics(best_weighted_bundle, EXTENDED_OUTPUT_DIR)
        effect_outputs = plot_prediction_learning_effects(best_weighted_bundle, EXTENDED_OUTPUT_DIR)
        teacher_outputs = plot_teacher_residual_learning_diagnostics(best_weighted_bundle, EXTENDED_OUTPUT_DIR)
        learning_workbook_path = save_learning_figure_workbook(
            screen_outputs,
            alpha_outputs,
            effect_outputs,
            teacher_outputs,
        ...
        print("Learning-process summary workbook:", learning_workbook_path)
    ...
    if best_weighted_record is not None:
        ...
        print("M3 test R2:", best_weighted_record["M2 test R2"])
